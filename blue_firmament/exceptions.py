...
class BlueFirmamentException(Exception, abc.ABC):
    ...
    def __init__(self, errmsg: str | dict = "Exception occured", *args, **kwargs):
        if isinstance(errmsg, str):
            errmsg = {"errmsg": errmsg, **kwargs}
    ...
    def dump_details_to_str(self) -> str:
        """将详细信息序列化为字符串"""
        return "\n".join([f"{k}: {v}" for k, v in self.errmsg.items()])
...
class ExternalError(BlueFirmamentException):
    ...
    def __init__(
        ...
        **errinfo: typing.Any,
        ...
        super().__init__({"service_name": service_name, **errinfo}, logger)
...
class Retryable(BlueFirmamentException):
    ...
    def __init__(self, delay: Opt[float] = None):
        """
...
class ParamsInvalid(ClientError, ValueError):
    ...
    def __init__(self, msg: str = "", **params):
        super().__init__(
            {"message": "Invalid parameter(s), %s" % msg, "parameters": params}
        )
...
class AtLeastOne(ParamsInvalid):
    ...
    def __str__(self) -> str:
        return (
            f"{self.name}: at least one of following parameters should be provided:\n"
            + "\n    ".join(self.__params)
        )
...
class InvalidStatusTransition(Conflict):
    ...
    def __init__(self, status_name: str, current_status, target_status):
        self.__status_name = status_name
        self.__current_status = current_status
        self.__target_status = target_status

    @classmethod
    def from_enum_member(cls, current: enum.Enum, target: enum.Enum) -> typing.Self:
        """从枚举成员创建"""
        return cls(current.__class__.__name__, current.value, target.value)
...
class Unauthorized(InternalError):
    ...
    def __init__(
        self, msg: Opt[str] = None, identity: Opt[str] = None, *args, **kwargs
    ):
        """
